function SwipeHandle(q, f) {
	...
	var G = {
		...
		transitions : (function(H) {
			var J = [ "transitionProperty", "WebkitTransition",
	...
	function p(I, L, K) {
		...
		J.webkitTransitionDuration = J.MozTransitionDuration = J.msTransitionDuration = J.OTransitionDuration = J.transitionDuration = K
				+ "ms";
		J.webkitTransform = "translate(" + L + "px,0)translateZ(0)";
		J.msTransform = J.MozTransform = J.OTransform = "translateX(" + L
				+ "px)"
	}
	function l(L, K, H) {
		if (!H) {
		...
		var I = setInterval(function() {
			...
			c.style.left = (((K - L) * (Math.floor((M / H) * 100) / 100)) + L)
					+ "px"
		}, 4)
	}
	var F = f.auto || 0;
		...
		move : function(H) {
			...
			if (!E) {
				...
				if (f.continuous) {
					...
					D.x = D.x
							/ ((!o && D.x > 0 || o == w.length - 1 && D.x < 0) ? (Math
									.abs(D.x)
									/ v + 1)
									: 1);
					p(o - 1, D.x + d[o - 1], 0);
			...
			var I = Number(L) < 250 && Math.abs(D.x) > 20
					|| Math.abs(D.x) > v / 2;
...
(function(f) {
	function p(v, A) {
		var z = (v & 65535) + (A & 65535), w = (v >> 16) + (A >> 16)
				+ (z >> 16);
		return (w << 16) | (z & 65535)
	}
	function t(v, w) {
...
(function(f) {
	...
	c = (f.os.fennec || f.feat.cssPrefix == "" || f.os.ie) ? "transitionend"
			: c;
	...
	var g = (function() {
		var k = function(l, m) {
			...
			this.animate(m);
			var o = this;
			jm(this.el).bind("destroy", function() {
		...
		k.prototype = {
			animate : function(w) {
				...
				this.isActive = true;
				if (!w) {
					alert("Please provide configuration options for animation of "
							+ this.el.id);
					return
				}
				var m = !!w.addClass;
				if (m) {
					...
					if (typeof (w.x) == "number"
							|| (w.x.indexOf("%") == -1
									&& w.x.toLowerCase().indexOf("px") == -1 && w.x
									.toLowerCase().indexOf("deg") == -1)) {
						w.x = parseInt(w.x) + "px"
					}
					if (typeof (w.y) == "number"
							|| (w.y.indexOf("%") == -1
									&& w.y.toLowerCase().indexOf("px") == -1 && w.y
									.toLowerCase().indexOf("deg") == -1)) {
						w.y = parseInt(w.y) + "px"
					}
					var v = "translate" + h + (w.x) + "," + (w.y) + e
							+ " scale(" + parseFloat(w.scale) + ") rotate("
							+ w.rotateX + ")";
					if (!f.os.opera) {
						v += " rotateY(" + w.rotateY + ")"
					}
				...
				if (u == 0 || (p == "ms" && u < 5) || l.display == "none") {
					...
					s.timeout = setTimeout(this.activeEvent,
							numOnly(w.time) + 50);
	...
	var a = (function() {
		...
		var e = function(g, i) {
			...
			try {
				...
				var h = this;
				this.handle.addEventListener("touchmove", function(l) {
					h.touchMove(l)
				}, false);
				this.handle.addEventListener("touchend", function(l) {
					h.touchEnd(l)
				}, false)
			} catch (k) {
		...
		e.prototype = {
			...
			touchStart : function(g) {
				...
				try {
					if (g.touches[0].target
							&& g.touches[0].target.type != undefined) {
					...
					this.maxTop = numOnly(this.el.style.height)
							- numOnly(this.handle.style.height);
					this.maxLeft = numOnly(this.el.style.width)
							- numOnly(this.handle.style.width);
			...
			touchEnd : function(k) {
				if (this.currentDrawer != null) {
					...
					if (this.direction == "up" || this.direction == "down") {
						...
						var h = Math.ceil(Math.abs(i) / Math.abs(this.maxTop)
								* 100);
						...
						var h = Math.ceil(Math.abs(i) / Math.abs(this.maxLeft)
								* 100);
						var g = numOnly(b.getCssMatrix(this.el).e);
						if (h > 17) {
							if (i > 0) {
			...
			drawerMove : function(g, k, i, h) {
				...
				g.style[b.feat.cssPrefix + "Transform"] = "translate" + d + k.x
						+ "px," + k.y + "px" + c;
...
(function(d) {
	...
	var e = (function() {
		...
		var h = function(q, i) {
			...
			var r = this;
			jm(this.container).bind(
					"destroy",
					function(u) {
						var v = r.container.jqmCarouselId;
						window.removeEventListener("orientationchange",
								r.orientationHandler, false);
						if (b[v]) {
							delete b[v]
						}
						u.stopPropagation()
			...
			this.refreshItems();
			var m = jm(l);
			m.bind("touchmove", function(u) {
				r.touchMove(u)
			});
			m.bind("touchend", function(u) {
				r.touchEnd(u)
			});
			m.bind("touchstart", function(u) {
				r.touchStart(u)
			});
		...
		h.prototype = {
			...
			touchStart : function(m) {
				...
				this.lockMove = false;
				if (m.touches[0].target
						&& m.touches[0].target.type !== undefined) {
			...
			touchMove : function(l) {
				...
				var i = this.vertical ? ((this.dy % this.myDivHeight)
				...
				if (!this.okToMove) {
					...
					this.okToMove = this.glue ? Math.abs(i) > this.glue
							&& Math.abs(i) < (100 - this.glue) : true;
			...
			onMoveIndex : function(k, m, r) {
				...
				if (r) {
					var i = r.width, y = r.height, x = y / i, q = this.container
							.querySelectorAll("img")[k], w = '<div class="photoListDetailLoadingDom" id="photoListDetailLoadingDom"><div class="photoLoading spin"></div></div>';
					if (this.myDivWidth > i && this.myDivHeight > y) {
						var t = this.myDivWidth / i > this.myDivHeight / y ? this.myDivHeight
								/ y
								: this.myDivWidth / i;
					...
					jm(q)
							.load(
									function() {
										jm(
												document
														.getElementById("photoListDetailLoadingDom"))
												.remove()
									});
					this.photoDetailAjax(this.photoAllJson[k]);
					this.photoListDetailShowDom
							.querySelector(".webPhotoListDetail").className = "webPhotoListDetail"
				}
				var u = false;
			...
			photoDetailAjax : function(k) {
				...
				var i = this.photoListDetailShowDom
						.querySelector(".drawer_handle"), l = i.parentNode
						.querySelector(".handleIcon");
				descriptionDom = document.getElementById("descriptionDom"
						+ this.moduleId);
				if (k.detail === undefined) {
					jm
							.ajax( {
								url : "ajax/photo_h.jsp",
								data : "cmd=getInfo&id=" + jm.encodeUrl(k.id)
										+ "&groupId=" + jm.encodeUrl(k.groupId),
								error : function() {
								},
								success : function(m) {
									m = m.trim();
									...
									descriptionDom.innerHTML = jm
											.encodeHtml(k.basic)
											+ (k.basic && m ? "<br /><br />"
													: "") + m;
									k.detail = m;
									if (descriptionDom.innerHTML) {
										descriptionDom.parentNode.style.bottom = i.offsetHeight
												* 2
												- descriptionDom.parentNode.offsetHeight
												+ "px";
					...
					descriptionDom.innerHTML = "";
					i.innerHTML = jm.encodeHtml(k.name);
					descriptionDom.innerHTML = jm.encodeHtml(k.basic)
							+ (k.basic && k.detail ? "<br /><br />" : "")
							+ k.detail;
					if (descriptionDom.innerHTML) {
						descriptionDom.parentNode.style.bottom = i.offsetHeight
								* 2 - descriptionDom.parentNode.offsetHeight
								+ "px";
						l.className = "handleIcon handleUp"
					} else {
						descriptionDom.parentNode.style.bottom = i.offsetHeight
								- descriptionDom.parentNode.offsetHeight + "px";
						l.className = "handleIcon"
					}
				}
			},
			moveCSS3 : function(i, m, l, k) {
				...
				i.style[d.feat.cssPrefix + "Transform"] = "translate" + g + m.x
						+ "px," + m.y + "px" + f;
...
(function(e) {
	...
	var f = [ "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11",
			"12" ], b = [ "日", "一", "二", "三", "四", "五", "六" ], i = /^(\+|\-)?(\d+)(M|Y)$/i, d = function(
			k, l) {
	...
	if (typeof LS != "undefined") {
		b = [ LS.date_Sun, LS.date_Mon, LS.date_Tue, LS.date_Wed, LS.date_Thu,
				LS.date_Fri, LS.date_Sat ]
	}
	var a = function(l, k) {
		this._options = {
			date : null,
	...
	a.prototype = {
		...
		_init : function(l) {
			...
			el = k._container || k.$el, k._initDateInput(el, l)
		},
		_hideDate : function(k, m) {
			var l = m._options, o = l.onAfterHidePicker;
		...
		_initDateInput : function(l, o) {
			var k = document.createElement("input"), m = this;
			...
			e(k).on(
					"click",
					function() {
						var t = m._options, s = m._container || m.$el, r = e
								.proxy(m._eventHandler, m), p = new Date(e(k)
								.val()), q = t.onBeforeInitCalendar, u;
						if (!!p.getTime()) {
							u = p
						} else {
							if (!!t.date) {
						...
						m.minDate(t.minDate).maxDate(t.maxDate).date(u)
								.refresh();
						s.addClass("ui-calendar").on("click", r);
						t.swipeable && s.on("swipeLeft swipeRight", r);
						t.slideAble
								&& s.on("touchstart touchend",
										".ui-calendar-calendar", r);
						setTimeout(function() {
							m._showDate(s)
						}, 100)
					})
		...
		_create : function() {
			...
			k.appendTo(this._options.container
					|| (k.parent().length ? "" : document.body))
		},
		_eventHandler : function(r) {
			var k = this._options, u = (this._container || this.$el).get(0), m = this, p, s, v, o, l, t, q;
			switch (r.type) {
			case "swipeLeft":
			case "swipeRight":
				return this.switchMonthTo((r.type == "swipeRight" ? "-" : "+")
						+ "1M");
			case "change":
				l = e(
						".ui-calendar-header .ui-calendar-year, .ui-calendar-header .ui-calendar-month",
						this._container);
			...
			default:
				s = r.target;
				if ((p = e(s).closest(".ui-calendar-calendar tbody a", u))
						&& p.length) {
					r.preventDefault();
					v = p.parent();
					this
							._option("selectedDate", o = new Date(v
									.attr("data-year"), v.attr("data-month"), p
									.text()));
					jm(this._container).trigger("select", o,
							e.calendar.formatDate(o), this);
					this.refresh();
					this._options._selectDate = this._options._selectedYear
							+ "-" + (this._options._selectedMonth + 1) + "-"
							+ this._options._selectedDay;
					typeof this._options.onSelectDate == "function"
							&& this._options.onSelectDate(
									this._options._selectDate, this);
					if (!this._options.showHour) {
						this._hideDate(this._container, this);
						this._input.val(this._options._selectDate)
					}
					typeof this._options.onAfterSelectDate == "function"
							&& this._options.onAfterSelectDate(
									this._options._selectDate, this)
				} else {
					if ((p = e(s).closest(
							".ui-calendar-prev, .ui-calendar-next", u))
							&& p.length) {
						r.preventDefault();
						this.switchMonthTo((p.is(".ui-calendar-prev") ? "-"
								: "+")
								+ "1M")
					} else {
						if (((p = e(s).closest(".ui-calendar-return", u)) || (p = e(
								s).closest(".ui-calendar-cancel", u)))
								&& p.length) {
							r.preventDefault();
							this._hideDate(this._container, this)
						}
		...
		_bindTimePickerEvent : function() {
			var p = this, o = 0, r = [ "height", "margin", "padding",
					"borderTopWidth", "borderBottomWidth" ], l, q, s, t;
			var k = (function() {
			...
			if (!k) {
				p._timePicker.container.find(".J_timeMaskWrap").css("z-index",
						"0");
				p._timePicker.container.find(".J_timeMark").addClass(
						"f-highLight")
			}
			...
			for ( var m = 0; m < r.length; m++) {
				...
				if (r[m] == "margin" || r[m] == "padding") {
					s = typeof s == "string" && s.length > 0 ? s.split(" ") : [
							"0px", "0px" ];
					...
					s = typeof s == "string" && s.length > 0 ? parseFloat(s
							.replace("px", "")) : 0;
					o += s
				}
			}
			p._timePicker.itemHeight = t.items[0].offsetHeight
					+ !!t.items.eq(0);
			l = new iScroll(
					"fk-timePickerPanel",
					{
						...
						onScrollStart : function() {
							p._timePicker.container
									.removeClass("J_endTimeFlag")
						},
						onScrollEnd : function() {
							if (p._timePicker.container
									.hasClass("J_endTimeFlag")) {
								return
							}
							var u = p._timePicker, A = u.items.eq(0), x = Math
									.abs(A.offset().top)
									+ parseFloat(A.height() / 2), y = [], z = u.timeMarkEL
									.offset().top
									+ t.timeMarkEL.height() / 2, C = u.container
									.find("li").not(".fk-timeBan"), B, w;
							for ( var v = 0; v < u.items.length; v++) {
								y.push(u.items.eq(v).offset().top
										+ parseFloat(A.height() / 2))
							}
							B = Math.abs(y[0] - z);
							w = 0;
							...
							p._options._selectTime = parseInt(C.eq(w).attr(
									"data-hour"));
							p._options.showHour
									&& p._container
											.find(".ui-calendar-navHeadHour")
											.text(
													(p._options._selectTime < 10 ? ("0" + p._options._selectTime)
															: p._options._selectTime)
															+ ":00")
						}
					});
			if (!!p._options._selectTime) {
				l
						.scrollTo(
								0,
								-p._timePicker.container.find("li")[p._options._selectTime].offsetTop,
								0)
			}
			p._container
					.find(".J_saveTimeBtn ")
					.on(
							...
							function(w) {
								...
								v = u == 0 ? "00:00"
										: (u < 10 ? ("0" + u + ":00") : u
												+ ":00");
								p._options._selectDate = p._options._selectedYear
										+ "-"
										+ (p._options._selectedMonth + 1)
										+ "-"
										+ p._options._selectedDay
										+ " "
										+ v;
		...
		_option : function(l, r) {
			...
			return l == "selectedDate" ? new Date(m._selectedYear,
					m._selectedMonth, m._selectedDay) : m[l]
		},
		_optionCash : {},
		...
		switchMonthTo : function(p, l) {
			var m = this._options, o = this.minDate(), s = this.maxDate(), r, q, k;
			if (Object.prototype.toString.call(p) === "[object String]"
					&& i.test(p)) {
				r = RegExp.$1 == "-" ? -parseInt(RegExp.$2, 10) : parseInt(
						RegExp.$2, 10);
			...
			if (p != m._drawMonth || l != m._drawYear) {
				jm(this._container).trigger("monthchange", m._drawMonth = p,
						m._drawYear = l, this);
				m._invalid = true;
				this.refresh()
			}
		...
		refresh : function() {
			var o = this, l = this._options, p = this._container || this.$el, w = l.onAfterRefresh, s = e
					.proxy(this._eventHandler, this), y, t, k, v, u, x, r, q, m;
			if (!l._invalid) {
				return
			}
			...
			if (l.slideAble && l._privateDate.slideAnimate) {
				...
				k = p.find(".ui-calendar-calendar-wrap").addClass(
				...
				setTimeout(function() {
					o._animateRunCtrl(k, "transform", "translateX(" + x + q
							+ "px)")
				}, 20);
			...
			p.find(".J_cancelBtn ").on("click", function(A) {
				var B = l.onBeforeCancel, z = o;
		...
		_animateRunCtrl : function(o, k, m) {
			var p = [ "", "-webkit-", "-moz-", "-ms-" ], l = p.length;
			while (l) {
				l--;
				o.css(p[l] + k, m)
			}
		...
		destroy : function() {
			var l = this._container || this.$el, m = this._input, k = this._eventHandler;
		...
		_generateHTML : function() {
			var k = this._options, r = k._drawYear, v = k._drawMonth, q = new Date(), z = new Date(
					q.getFullYear(), q.getMonth(), q.getDate()), t = this
					.minDate(), o = this.maxDate(), x = this.selectedDate(), w = "", u, s, m, A, y, l, B, p;
			m = (isNaN(m = parseInt(k.firstDay, 10)) ? 0 : m);
			w += this._renderNavHeader(k);
			w += this._renderHead(k, r, v, t, o)
					+ '<div class="ui-calendar-calendar-wrap"><table  class="ui-calendar-calendar"><thead><tr>';
			for (u = 0; u < 7; u++) {
				A = (u + m) % 7;
				w += "<th"
						+ ((u + m + 6) % 7 >= 5 ? ' class="ui-calendar-week-end"'
								: "") + "><span>" + b[A] + "</span></th>"
			}
			w += "</thead></tr><tbody>";
			l = d(r, v);
		...
		_renderHead : function(q, m, r, o, k) {
			var s = '<div class="ui-calendar-header">', l = new Date(m, r, -1), u = new Date(
					m, r + 1, 1), w = (!!o && typeof o.getFullYear == "function") ? o
					.getFullYear()
					: null, t = (!!o && typeof o.getMonth == "function") ? o
					.getMonth() : null, v = p + 58, v = v <= 2028 ? v : 2028, p;
			s += '<a class="ui-calendar-return"></a><div class="ui-calendar-title">';
			if (q.yearChangeable) {
				s += '<select class="ui-calendar-year">';
				for (p = (w != null && w < m ? w : m); p < v; p++) {
					s += '<option value="' + p + '" '
							+ (p == m ? 'selected="selected"' : "") + ">" + p
							+ "</option>"
				}
				s += "</select>"
			} else {
				s += '<span class="ui-calendar-year" data-value="' + m + '">'
						+ m + "年</span>"
			}
			if (q.monthChangeable) {
				...
				s += '<span class="ui-calendar-month" data-value="' + r + '">'
						+ f[r] + "</span>"
			}
			s += '</div><div class="ui-calendar-preandnext"><a class="ui-calendar-prev'
					+ (o && o > l ? " ui-state-disable" : "")
					+ '" href="#"></a><a class="ui-calendar-next'
					+ (k && k < u ? " ui-state-disable" : "")
					+ '" href="#"></a></div></div>';
			return s
		},
		_renderDay : function(r, o, l, s, t, u, q, k) {
			var m = (o.getMonth() !== s), v = this._options.beforeShowDay, p;
			p = ((typeof v == "function" && !v(o, this))) || m || (q && o < q)
					|| (k && o > k);
			return "<td class='"
					+ ((r + l + 6) % 7 >= 5 ? "ui-calendar-week-end" : "")
					+ (p ? " ui-calendar-unSelectable ui-state-disabled" : "")
					+ (m || p ? ""
							: (o.getTime() === t.getTime() ? " ui-calendar-current-day"
									: "")
									+ (o.getTime() === u.getTime() ? " ui-calendar-today"
											: ""))
					+ "'"
					+ (p ? "" : " data-month='" + o.getMonth()
					...
					+ (m ? "&#xa0;"
							: (p ? "<span class='ui-state-default'>"
									...
									: "<a class='ui-state-default"
											+ (o.getTime() === u.getTime() ? " ui-state-highlight"
													: "")
											+ (o.getTime() === t.getTime() ? " ui-state-active"
													: "") + "' href='#'>"
											+ o.getDate() + "</a>")) + "</td>"
		},
		_renderTime : function() {
			var o = "", m = this._options, k = false, p = m.beforeShowTime;
			if (!m.showHour) {
				o += "<div class='fk-timePickerSubmit'>";
				o += "<div class='J_cancelBtn f-dateCancelBtn'>" + LS.cancel
						+ "</div>";
				o += "</div>";
				return o
			}
			...
			for ( var l = 0; l <= 23; l++) {
				...
				o += "<li class='" + (k ? "fk-timeBan" : "") + "' data-hour='"
						+ l + "'>" + (l < 10 ? ("0" + l) : l) + ":00</li>"
			}
			o += "<li>&nbsp</li>";
			o += "<li>&nbsp</li>";
	...
	e.each( [ "maxDate", "minDate", "date", "selectedDate" ], function(l, k) {
	...
	e.calendar = {
		parseDate : function(l) {
			...
			return Object.prototype.toString.call(l) === "[object Date]" ? l
					: k.test(l) ? new Date(parseInt(RegExp.$1, 10), parseInt(
							RegExp.$2, 10) - 1, parseInt(RegExp.$3, 10)) : null
		},
		formatDate : function(k) {
			return k.getFullYear() + "-" + c(k.getMonth() + 1, 2) + "-"
					+ c(k.getDate(), 2)
		}
	}
})(jm);
/*
 * ! iScroll Lite base on iScroll v4.1.6 ~ Copyright (c) 2011 Matteo Spinelli,
 * http://cubiq.org Released under MIT license, http://cubiq.org/license
 */
(function() {
	var s = Math, d = function(m) {
		return m >> 0
	}, w = (/webkit/i).test(navigator.appVersion) ? "webkit" : (/firefox/i)
			.test(navigator.userAgent) ? "Moz" : "opera" in window ? "O" : "", x = (/android/gi)
			.test(navigator.appVersion), i = (/iphone|ipad/gi)
			.test(navigator.appVersion), c = (/playbook/gi)
			.test(navigator.appVersion), p = (/hp-tablet/gi)
			.test(navigator.appVersion), l = "WebKitCSSMatrix" in window
			&& "m11" in new WebKitCSSMatrix(), v = "ontouchstart" in window
			&& !p, f = w + "Transform" in document.documentElement.style, g = i
			|| c, q = (function() {
		return window.requestAnimationFrame
				|| window.webkitRequestAnimationFrame
				|| window.mozRequestAnimationFrame
				|| window.oRequestAnimationFrame
				|| window.msRequestAnimationFrame || function(m) {
	...
	})(), h = "onorientationchange" in window ? "orientationchange" : "resize", b = v ? "touchstart"
			: "mousedown", r = v ? "touchmove" : "mousemove", e = v ? "touchend"
			: "mouseup", u = v ? "touchcancel" : "mouseup", a = "translate"
			+ (l ? "3d(" : "("), k = l ? ",0)" : ")", t = function(z, m) {
		var A = this, B = document, y;
		...
		A.scroller.style[w + "TransitionProperty"] = A.options.useTransform ? "-"
				+ w.toLowerCase() + "-transform"
				: "top left";
		...
		if (A.options.useTransform) {
			A.scroller.style[w + "Transform"] = a + A.x + "px," + A.y + "px"
					+ k
		} else {
			A.scroller.style.cssText += ";position:absolute;top:" + A.y
					+ "px;left:" + A.x + "px"
		}
		A.refresh();
		A._bind(h, window);
	...
	t.prototype = {
		...
		_pos : function(m, z) {
			...
			if (this.options.useTransform) {
				this.scroller.style[w + "Transform"] = a + m + "px," + z + "px"
						+ k + " scale(" + this.scale + ")"
			} else {
				m = d(m);
				z = d(z);
			...
			if (this.options.remarkXCoordinate) {
				...
				if (Object.prototype.toString
						.call(this.options.remarkNavLeftRight) === "[object Function]") {
					this.options.remarkNavLeftRight(this.x, this)
				}
			}
		},
		_start : function(C) {
			var B = this, z = v ? C.touches[0] : C, A, m, D;
			if (!B.enabled) {
				return
			}
			...
			if (B.options.momentum) {
				if (B.options.useTransform) {
					A = getComputedStyle(B.scroller, null)[w + "Transform"]
							.replace(/[^0-9-.,]/g, "").split(",");
					m = A[4] * 1;
					D = A[5] * 1
				} else {
					m = getComputedStyle(B.scroller, null).left.replace(
							/[^0-9-]/g, "") * 1;
					D = getComputedStyle(B.scroller, null).top.replace(
							/[^0-9-]/g, "") * 1
				}
				if (m != B.x || D != B.y) {
					if (B.options.useTransition) {
		...
		_move : function(D) {
			var A = this, y = v ? D.touches[0] : D, z = y.pageX - A.pointX, m = y.pageY
					- A.pointY, E = A.x + z, C = A.y + m, B = D.timeStamp
					|| Date.now();
			...
			if (E > 0 || E < A.maxScrollX) {
				E = A.options.bounce ? A.x + (z / 2) : E >= 0
						|| A.maxScrollX >= 0 ? 0 : A.maxScrollX
			}
			if (C > 0 || C < A.maxScrollY) {
				C = A.options.bounce ? A.y + (m / 2) : C >= 0
						|| A.maxScrollY >= 0 ? 0 : A.maxScrollY
			}
			A.distX += z;
			A.distY += m;
		...
		_end : function(D) {
			...
			var B = this, H = v ? D.changedTouches[0] : D, E, G, z = {
				dist : 0,
				time : 0
			}, m = {
				dist : 0,
				time : 0
			}, A = (D.timeStamp || Date.now()) - B.startTime, F = B.x, C = B.y, y;
			B._unbind(r);
			B._unbind(e);
			B._unbind(u);
			...
			if (!B.moved) {
				if (v) {
					...
					if (E.tagName != "SELECT" && E.tagName != "INPUT"
						...
						G.initMouseEvent("click", true, true, D.view, 1,
								H.screenX, H.screenY, H.clientX, H.clientY,
								D.ctrlKey, D.altKey, D.shiftKey, D.metaKey, 0,
								null);
						G._fake = true;
						E.dispatchEvent(G)
					}
			...
			if (A < 300 && B.options.momentum) {
				z = F ? B._momentum(F - B.startX, A, -B.x, B.scrollerW
						- B.wrapperW + B.x, B.options.bounce ? B.wrapperW : 0)
						: z;
				m = C ? B
						._momentum(C - B.startY, A, -B.y,
								(B.maxScrollY < 0 ? B.scrollerH - B.wrapperH
										+ B.y : 0),
								B.options.bounce ? B.wrapperH : 0) : m;
				F = B.x + z.dist;
				C = B.y + m.dist;
				if ((B.x > 0 && F > 0)
						|| (B.x < B.maxScrollX && F < B.maxScrollX)) {
					z = {
						dist : 0,
						time : 0
					}
				...
				if ((B.y > 0 && C > 0)
						|| (B.y < B.maxScrollY && C < B.maxScrollY)) {
					m = {
						dist : 0,
						time : 0
					}
		...
		_resetPos : function(z) {
			var m = this, A = m.x >= 0 ? 0 : m.x < m.maxScrollX ? m.maxScrollX
					: m.x, y = m.y >= 0 || m.maxScrollY > 0 ? 0
					: m.y < m.maxScrollY ? m.maxScrollY : m.y;
			if (A == m.x && y == m.y) {
				if (m.moved) {
		...
		_startAni : function() {
			var D = this, y = D.x, m = D.y, B = Date.now(), C, A, z;
			if (D.animating) {
				return
			}
			if (!D.steps.length) {
		...
		_momentum : function(E, y, C, m, G) {
			var D = 0.0006, z = s.abs(E) / y, A = (z * z) / (2 * D), F = 0, B = 0;
			if (E > 0 && A > C) {
				B = G / (6 / (A / z * D));
				C = C + B;
				z = z * C / A;
		...
		_offset : function(m) {
			var z = -m.offsetLeft, y = -m.offsetTop;
		...
		scrollToElement : function(m, z) {
			...
			A.left = A.left > 0 ? 0 : A.left < y.maxScrollX ? y.maxScrollX
					: A.left;
			A.top = A.top > 0 ? 0 : A.top < y.maxScrollY ? y.maxScrollY : A.top;
			z = z === undefined ? s.max(s.abs(A.left) * 2, s.abs(A.top) * 2)
					: z;
...
/*
 * ! klass: a classical JS OOP façade https://github.com/ded/klass License MIT
...
!function(a, c, b) {
	...
	var c = this, d = c.klass, h = "function", l = /xyz/.test(function() {
		xyz
	}) ? /\bsupr\b/ : /.*/, e = "prototype";
	function i(f) {
	...
	function a(p, q, f) {
		for ( var m in q) {
			if (q.hasOwnProperty(m)) {
				p[m] = g(q[m]) && g(f[e][m]) && l.test(q[m]) ? b(m, q[m], f)
						: q[m]
			}
		}
	}
	function k(p, r) {
		...
		v[e] = this[e];
		var s = this, u = new v(), q = g(p), f = q ? p : this, m = q ? {} : p;
		function t() {
			if (this.initialize) {
...
(function(a) {
	if (!Function.prototype.bind) {
		Function.prototype.bind = function(e) {
			var f = [].slice, c = f.call(arguments, 1), b = this, g = function() {
			}, d = function() {
				return b.apply(this instanceof g ? this : (e || {}), c.concat(f
						.call(arguments)))
			};
...
(function(a, b) {
	b.Browser = {
		...
		_detect : function() {
			...
			this.isGestureSupported = document.documentElement.ongesturestart !== "undefined" ? true
					: false
		},
		_eventTagNames : {
			select : "input",
					...
					b,
					{
						DOM : {
							...
							find : function(g, h) {
								...
								var f = h.querySelectorAll(g), c = [], e, d;
							...
							_addClass : function(f, e) {
								var c = b.DOM.getAttribute(f, "class", ""), d = new RegExp(
										"(?:^|\\s+)" + e + "(?:\\s+|$)");
								if (!d.test(c)) {
									if (c !== "") {
										c = c + " "
							...
							_removeClass : function(h, g) {
								var c = b.DOM.getAttribute(h, "class", ""), f = b
										.trim(c).split(" "), k = "", e, d;
								for (e = 0, d = f.length; e < d; e++) {
									if (f[e] !== g) {
										if (k !== "") {
							...
							_hide : function(c) {
								b.DOM.setData(c, "ccl-disp", b.DOM.getStyle(c,
										"display"));
							...
							_show : function(c) {
								if (b.DOM.getStyle(c, "display") === "none") {
									var d = b.DOM.getData(c, "ccl-disp",
											"block");
									if (d === "none" || d === "") {
										d = "block"
									}
							...
							_getDimension : function(e, f) {
								var c = a.parseInt(a.getComputedStyle(e, "")
										.getPropertyValue(f)), d;
								if (isNaN(c)) {
									d = {
									...
									c = a.parseInt(a.getComputedStyle(e, "")
											.getPropertyValue(f));
					...
					b,
					{
						Animation : {
							...
							resetTranslate : function(e) {
								var f = e.nodeName || "", d = (f.toLowerCase() === "img") ? true
										: false, c = {};
								if (!d) {
							...
							_leaveTransforms : function(f) {
								var m = f.style[this._transitionPrefix
										+ "Property"], l = (m !== "") ? "ccl"
										+ m + "callback" : "cclallcallback", p, g = b
										.coalesce(f.style.webkitTransform,
												f.style.MozTransform,
												f.style.transform), i, o, k = a
						...
						c.Events
								.fire(
										this,
										{
											...
											action : (f < 0) ? c.TouchElement.ActionTypes.swipeLeft
...
(function(c, a, d) {
	...
	b.Image.ImageClass = a( {
		...
		load : function() {
			...
			var e = this.refObj.getAttribute("data-source") ? this.refObj
					.getAttribute("data-source") : this.refObj.src;
...
(function(c, a, d) {
	...
	b.DocumentOverlay.DocumentOverlayClass = a( {
		...
		resetPosition : function() {
			...
			if (this.settings.target === c) {
				f = d.DOM.windowWidth();
				e = d.DOM.bodyOuterHeight() * 2;
				g = (this.settings.jQueryMobile) ? d.DOM.windowScrollTop()
						+ "px" : "0px";
				if (e < 1) {
...
(function(c, a, d) {
	...
	b.Carousel.CarouselClass = a( {
		...
		resetImagePosition : function(g) {
			...
			var e = d.DOM.getAttribute(g, "src"), h, i, f, k, l, o = d.DOM
					.width(this.el), m = d.DOM.height(this.el);
				...
				doMoveCarousel : function(g) {
					...
					if (e.Browser.isCSSTransformSupported) {
						...
						if (!e.isNothing(d.jQuery)) {
							d.jQuery(this.contentEl).stop().css("left",
									this.getStartingPos().x + g + "px")
						}
					}
				},
				doSlideCarousel : function(i, h) {
					...
					if (e.Browser.isCSSTransformSupported) {
						...
						if (!e.isNothing(d.jQuery)) {
							...
							if (e
									.isNothing(d.jQuery.easing[this.settings.animationTimingFunction])) {
								this.settings.animationTimingFunction = "linear"
							}
							d.jQuery(this.contentEl).animate(f,
	...
	b.Toolbar.ToolbarClass = a( {
		...
		initialize : function(f, g) {
			...
			this.previousEl = d.DOM.find("." + b.Toolbar.CssClasses.previous,
					this.toolbarEl)[0];
			this.showDetailEl = d.DOM.find("."
					+ b.Toolbar.CssClasses.showDetailEl, this.toolbarEl)[0];
		...
		resetPosition : function() {
			...
			if (this.settings.target === c) {
				if (this.settings.captionAndToolbarFlipPosition) {
					...
					g = (d.DOM.windowScrollTop() + d.DOM.windowHeight())
				...
				if (this.settings.captionAndToolbarFlipPosition) {
					e = "0";
					g = d.DOM.height(this.settings.target)
							- d.DOM.height(this.captionEl)
				} else {
					e = d.DOM.height(this.settings.target)
							- d.DOM.height(this.toolbarEl);
				...
				if (g.target === this.previousEl
					...
					if (g.target === this.closeEl
						...
						if (g.target === this.playEl
							...
							if (g.target === this.showDetailEl
									|| d.DOM.isChildOf(g.target,
											this.showDetailEl)) {
...
(function(c, a, d) {
	d.registerNamespace("Code.PhotoSwipe.UILayer");
	var b = c.Code.PhotoSwipe;
	b.UILayer.UILayerClass = d.TouchElement.TouchElementClass
			.extend( {
				el : null,
				settings : null,
				dispose : function() {
...
(function(c, a, d) {
	d.registerNamespace("Code.PhotoSwipe.UIDetail");
	var b = c.Code.PhotoSwipe;
	b.UIDetail.UIDetailClass = d.TouchElement.TouchElementClass
			.extend( {
				el : null,
				settings : null,
				dispose : function() {
				...
				initialize : function(e) {
					...
					d.DOM.appendChild(this.elChlidTitleName,
							this.elChlidTitlePanel);
					d.DOM
							.appendChild(this.elChlidTitle,
									this.elChlidTitlePanel);
				...
				show : function() {
					this.resetPosition();
					if (Object.prototype.toString
							.call(this.settings.uiDetailCallBack) === "[object Function]") {
						this.settings.uiDetailCallBack(this.elChlidContent,
								this.elChlid, this.elChlidTitleName)
					}
					d.DOM.show(this.el);
					this.addEventHandlers()
				},
...
(function(b, c) {
	...
	a.attach = function(m, p, e) {
		...
		for (k = 0, h = m.length; k < h; k++) {
			...
			if (!c.isNothing(g.nodeType)) {
				if (jm(g)
						.parents(
								"#newsDetailPanel a, .richContent  a, .productDetailTabPanel a").length >= 1) {
				} else {
					if (g.parentNode
							&& (g.parentNode.nodeName === "A"
									&& g.parentNode.href || g.parentNode.parentNode.nodeName === "A"
									&& g.parentNode.parentNode.href)) {
						continue
					}
					if (g.nodeType === 1) {
						g.__photoSwipeClickHandler = a.onTriggerElementClick
								.bind(o);
	...
	a.createInstance = function(e, f, k) {
		...
		if (c.isNothing(d)) {
			...
			throw 'Code.PhotoSwipe.createInstance: Instance with id "' + k
					+ ' already exists."'
		}
		return d
	};
	...
	a.PhotoSwipeClass = g( {
		...
		initialize : function(m, o, q) {
			...
			if (this.settings.target !== f) {
				...
				if (p !== "relative" || p !== "absolute") {
					i.DOM
							.setStyle(this.settings.target, "position",
									"relative")
				}
			}
			if (this.settings.target !== f) {
		...
		resetPosition : function() {
			...
			if (!i.isNothing(this.windowDimensions)) {
				if (m.width === this.windowDimensions.width
						&& m.height === this.windowDimensions.height) {
					return
				}
			}
		...
		addEventHandlers : function() {
			...
			if (i.Browser.android) {
				...
				if (i.Browser.iOS && (!i.Browser.safari)) {
					...
					this.orientationEventName = m ? "orientationchange"
							: "resize"
				}
			}
		...
		onDocumentOverlayFadeIn : function(m) {
			f.setTimeout(function() {
				var o = (this.settings.target === f) ? f.document.body
			...
			if (this.isZoomActive()) {
				...
				case i.TouchElement.ActionTypes.gestureChange:
					this.zoomPanRotate
							.zoomRotate(
									t.scale,
									(this.settings.allowRotationOnUserZoom) ? t.rotation
											: 0);
					break;
				...
				case i.TouchElement.ActionTypes.doubleTap:
					...
					var q = this.cache.images[this.currentIndex].imageEl, s = f
							.parseInt(i.DOM.getStyle(q, "top"), 10), m = f
							.parseInt(i.DOM.getStyle(q, "left"), 10), o = m
							+ i.DOM.width(q), p = s + i.DOM.height(q);
					if (t.point.x < m) {
						t.point.x = m
					} else {
						if (t.point.x > o) {
					...
					if (this.isZoomActive()) {
						this.zoomPanRotate.zoomAndPanToPoint(
								this.settings.doubleTapZoomLevel, t.point)
					}
					break;
		...
		onCarouselSlideByEnd : function(m) {
			...
			if (this.settings.resetToolbarPosition) {
				if (this.settings.photoJson) {
					this.toolbar
							.insertToolbarText(this.settings.photoJson[this.currentIndex])
				}
				this.toolbar.resetPosition()
			}
		},
		...
		onMouseWheel : function(o) {
			var p = i.Events.getWheelDelta(o), m = o.timeStamp
		...
		window.Code.PhotoSwipe.UILayer, window.Code.PhotoSwipe.UIDetail,
...
(function(c, b, a, e) {
	...
	c.fn.lazyload = function(f) {
						...
						o.on(g.otherEvent,
								function() {
									if (!g.load_in_viewport) {
										...
										if (c.abovethetop(m, g)
												|| c.leftofbegin(m, g)) {
										} else {
											if (!c.belowthefold(m, g)
													&& !c.rightoffold(m, g)) {
												o.trigger("appear")
											}
										}
		...
		d.on("resize.lazyload", function() {
			l()
		});
		if ((/(?:iphone|ipod|ipad).*os 5/gi).test(navigator.appVersion)) {
			d.on("pageshow", function(m) {
	...
	c.belowthefold = function(g, h) {
		...
		if (h.container === e || h.container === b) {
			f = (b.innerHeight ? b.innerHeight : d.height())
					+ (a.documentElement.scrollTop || b.pageYOffset || a.body.scrollTop)
		} else {
	...
	c.rightoffold = function(g, h) {
		...
		if (h.container === e || h.container === b) {
			f = d.width()
					+ (a.documentElement.scrollLeft || b.pageXOffset || a.body.scrollLeft)
		} else {
			i = c(h.container).offset();
			f = i.left + i.width
		}
	...
	c.abovethetop = function(g, h) {
		var f, i;
		if (h.container === e || h.container === b) {
			f = a.documentElement.scrollTop || b.pageYOffset
					|| a.body.scrollTop
		} else {
