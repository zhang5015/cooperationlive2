if (!window.jm || typeof (jm) !== "function") {
	var jm = (function(O) {
		var r, j = O.document, P = [], v = P.slice, Q = {}, q = [], k = 1, n = [], d = 1, M = /^\s*<(\w+)[^>]*>/, s = {}, B = {};
		function J(Y, Z, ab) {
		...
		p.fn = D.prototype = {
			...
			ready : function(Y) {
				if (j.readyState === "complete" || j.readyState === "loaded"
						|| (!p.os.ie10 && j.readyState === "interactive")) {
					Y()
				} else {
			...
			css : function(ac, ad, af) {
				...
				if (ad == r && typeof (ac) === "string") {
					...
					return ae.style[ac] ? ae.style[ac] : ab
							.getPropertyValue(ac)
				}
			...
			hide : function() {
				...
				for ( var Y = 0; Y < this.length; Y++) {
					if (this.css("display", null, this[Y]) != "none") {
						this[Y].setAttribute("jqmOldStyle", this.css("display",
								null, this[Y]));
				...
				for ( var Y = 0; Y < this.length; Y++) {
					if (this.css("display", null, this[Y]) == "none") {
						this[Y].style.display = this[Y]
								.getAttribute("jqmOldStyle") ? this[Y]
								.getAttribute("jqmOldStyle") : "block";
			...
			toggle : function(Z) {
				...
				for ( var aa = 0; aa < this.length; aa++) {
					if (O.getComputedStyle(this[aa])["display"] !== "none"
							|| (Z !== r && Y === false)) {
						this[aa].setAttribute("jqmOldStyle",
								this[aa].style.display);
						this[aa].style.display = "none"
					} else {
						this[aa].style.display = this[aa]
								.getAttribute("jqmOldStyle") != r ? this[aa]
								.getAttribute("jqmOldStyle") : "block";
			...
			attr : function(Y, ab) {
				...
				if (ab === r && !p.isObject(Y)) {
					var ac = (this[0].jqmCacheId && s[this[0].jqmCacheId][Y]) ? (this[0].jqmCacheId && s[this[0].jqmCacheId][Y])
			...
			prop : function(ad, ab) {
				...
				if (ab === r && !p.isObject(ad)) {
					var aa;
					var ac = (this[0].jqmCacheId && B[this[0].jqmCacheId][ad]) ? (this[0].jqmCacheId && B[this[0].jqmCacheId][ad])
							: !(aa = this[0][ad]) && ad in this[0] ? this[0][ad]
									: aa;
					return ac
				}
				for ( var Z = 0; Z < this.length; Z++) {
			...
			remove : function(Y) {
				...
				for ( var aa = 0; aa < Z.length; aa++) {
					...
					if (typeof (Z[aa].parentNode) != "undefined"
							&& Z[aa].parentNode != null) {
			...
			replaceClass : function(Z, Y) {
				for ( var aa = 0; aa < this.length; aa++) {
					...
					Z.split(/\s+/g).concat(Y.split(/\s+/g)).forEach(
							function(ac) {
								ab = ab.replace(U(ac), " ")
							});
					ab = ab.trim();
			...
			insertBefore : function(Z, aa) {
				...
				for ( var Y = 0; Y < this.length; Y++) {
					aa ? Z.parentNode.insertBefore(this[Y], Z.nextSibling)
			...
			siblings : function(Y) {
				...
				for ( var aa = 0; aa < this.length; aa++) {
					if (this[aa].parentNode) {
						Z = Z
								.concat(S(this[aa].parentNode.firstChild,
										this[aa]))
					}
				}
				return this.setupOld(p(Z).filter(Y))
			...
			index : function(Y) {
				return Y ? this.indexOf(p(Y)[0]) : this.parent().children()
						.indexOf(this[0])
			},
			is : function(Y) {
		...
		p.ajax = function(Y) {
			...
			try {
				...
				ab.data = ab.data ? (ab.data + "&_TOKEN=" + p("#_TOKEN").attr(
						"value")) : ("_TOKEN=" + p("#_TOKEN").attr("value"));
				...
				af.onreadystatechange = function() {
					...
					if (af.readyState === 4) {
						clearTimeout(ad);
						var ah, ai = false;
						if ((af.status >= 200 && af.status < 300)
								|| af.status === 0 && ag == "file:") {
							if (aj === "application/json"
									&& !(/^\s*$/.test(af.responseText))) {
								try {
		...
		p.param = function(ac, aa) {
			...
			if (ac instanceof D) {
				ac.each(function() {
					var af = aa ? aa + "[]" : this.id, ae = this.value;
					ad.push((af) + "=" + encodeURIComponent(ae))
				})
			} else {
				for ( var ab in ac) {
					var Z = aa ? aa + "[" + ab + "]" : ab, Y = ac[ab];
					ad.push(p.isObject(Y) ? p.param(Y, Z) : (Z) + "="
		...
		function h(ab, ae) {
			...
			ab.os.WINDOW_DESKTOP = ae.match(/Windows NT/i)
					&& !ae.match(/IEMobile/i) ? true : false;
			ab.os.webkit = ae.match(/WebKit\/([\d.]+)/) ? true : false;
			ab.os.android = ae.match(/(Android)\s+([\d.]+)/)
					|| ae.match(/Silk-Accelerated/) ? true : false;
			ab.os.androidICS = ab.os.android && ae.match(/(Android)\s4/) ? true
					: false;
			ab.os.ipad = ae.match(/(iPad).*OS\s([\d_]+)/) ? true : false;
			ab.os.iphone = !ab.os.ipad && ae.match(/(iPhone\sOS)\s([\d_]+)/) ? true
					: false;
			ab.os.webos = ae.match(/(webOS|hpwOS)[\s\/]([\d.]+)/) ? true
					: false;
			...
			ab.os.blackberry = ab.os.playbook || ae.match(/BlackBerry/) ? true
					: false;
			ab.os.blackberry10 = ab.os.blackberry && ae.match(/Safari\/536/) ? true
					: false;
			ab.os.chrome = ae.match(/Chrome/) ? true : false;
			ab.os.safari = navigator.userAgent.match(/(iPod|iPhone|iPad)/)
					&& navigator.userAgent.match(/AppleWebKit/) ? true : false;
			var ad = Y();
			...
			ab.os.BaiDuBrowser = ae.match(/baidubrowser|BIDUBrowser/i) ? true
					: false;
			...
			ab.os.fennec = ae.match(/fennec/i) ? true
					: ae.match(/Firefox/) ? true : false;
			ab.os.ie = (ae.match(/Trident/i) && (ae.match(/rv:11/i)) || (ae)
					.match(/MSIE/i)) ? true : false;
			ab.os.ie9 = ae.match(/MSIE 9.0/i) ? true : false;
			ab.os.ie10 = ae.match(/MSIE 10.0/i) ? true : false;
			ab.os.ie11 = (ae.match(/Trident/i) && (ae.match(/rv:11/i)) || (ae)
					.match(/MSIE 11.0/i)) ? true : false;
			ab.os.ieTouch = ab.os.ie && ae.toLowerCase().match(/touch/i) ? true
					: false;
			...
			ab.feat.nativeTouchScroll = typeof (ac.style["-webkit-overflow-scrolling"]) !== "undefined"
					&& ab.os.ios;
			ab.feat.cssPrefix = ab.os.webkit ? "Webkit" : ab.os.fennec ? "Moz"
					: ab.os.ie ? "ms" : ab.os.opera ? "O" : "";
			...
			function Y() {
				if (/iP(hone|od|ad)/.test(navigator.platform)) {
					var af = (navigator.appVersion)
							.match(/OS (\d+)_(\d+)_?(\d+)?/);
					return af ? [ parseInt(af[1], 10), parseInt(af[2], 10),
							parseInt(af[3] || 0, 10) ] : [ 0, 0 ]
				}
				return [ 0, 0 ]
			}
			function aa() {
		...
		p.getCookie = function(Y) {
			if (j.cookie.length > 0) {
				...
				if (c_start != -1) {
					...
					return decodeURIComponent(j.cookie
		...
		p.setCookie = function(Z, aa, Y) {
			...
			j.cookie = Z + "=" + encodeURIComponent(aa)
					+ ((Y == null) ? "" : ";expires=" + ab.toGMTString())
		};
		p.cookie = function(ab, ac, Z) {
			if (arguments.length > 1 && (ac === null || typeof ac !== "object")) {
				...
				if (typeof Z.expires === "number") {
					var ae = Z.expires, aa = Z.expires = new Date();
					aa.setDate(aa.getDate() + ae)
				}
				return (j.cookie = [
						encodeURIComponent(ab),
						"=",
						Z.raw ? String(ac) : encodeURIComponent(String(ac)),
						Z.expires ? "; expires=" + Z.expires.toUTCString() : "",
						Z.path ? "; path=" + Z.path : "",
						Z.domain ? "; domain=" + Z.domain : "",
						Z.secure ? "; secure" : "" ].join(""))
			}
			Z = ac || {};
			var Y, ad = Z.raw ? function(af) {
				return af
			} : decodeURIComponent;
			return (Y = new RegExp("(?:^|; )" + encodeURIComponent(ab)
					+ "=([^;]*)").exec(j.cookie)) ? ad(Y[1]) : null
		};
		p.uuid = function() {
			var Y = function() {
			...
			try {
				if (O.WebKitCSSMatrix) {
					return new WebKitCSSMatrix(
							O.getComputedStyle(Z).webkitTransform)
				} else {
					if (O.MSCSSMatrix) {
						return new MSCSSMatrix(O.getComputedStyle(Z).transform)
					} else {
						var Y = O.getComputedStyle(Z)[p.feat.cssPrefix
		...
		function g(ab, aa, ac, Z, Y) {
			var ae = z(ab), ad = (c[ae] || (c[ae] = []));
			u(aa, ac, function(ai, ah) {
				var ag = Y && Y(ah, ai), ak = ag || ah;
				var aj = function(am) {
				...
				g(Z, ab, ac, Y, function(ad) {
					return function(ag) {
						...
						if (af) {
							...
							return ad.apply(af, [ ae ].concat( [].slice.call(
									arguments, 1)))
						}
					}
				})
		...
		p.fn.on = function(Z, Y, aa) {
			return Y === r || p.isFunction(Y) ? this.bind(Z, Y) : this
					.delegate(Y, Z, aa)
		};
		p.fn.off = function(Z, Y, aa) {
			return Y === r || p.isFunction(Y) ? this.unbind(Z, Y) : this
					.undelegate(Y, Z, aa)
		};
		...
		p.Event = function(ab, aa) {
			var ac = j.createEvent("Events"), Y = true;
			if (aa) {
				for ( var Z in aa) {
		...
		p.toJSON = function(aa) {
			...
			if (ah === "date") {
				var af = aa.getUTCMonth() + 1, ai = aa.getUTCDate(), ag = aa
						.getUTCFullYear(), aj = aa.getUTCHours(), ac = aa
						.getUTCMinutes(), ak = aa.getUTCSeconds(), ae = aa
						.getUTCMilliseconds();
				if (af < 10) {
					af = "0" + af
				}
		...
		p.quoteString = function(Y) {
			if (Y.match(A)) {
				return '"'
						+ Y.replace(A, function(Z) {
							var aa = m[Z];
							if (typeof aa === "string") {
								return aa
							}
							aa = Z.charCodeAt();
							return "\\u00" + Math.floor(aa / 16).toString(16)
									+ (aa % 16).toString(16)
						}) + '"'
			}
			return '"' + Y + '"'
		...
		p.parseJSON = function(Y) {
			...
			if (rvalidchars.test(Y.replace(rvalidescape, "@").replace(
					rvalidtokens, "]").replace(rvalidbraces, ""))) {
				return (new Function("return " + Y))()
			}
		};
		var L = /^\s+/, y = /\s+$/, a = String.prototype.trim;
		p.trim = a ? function(Y) {
		...
		p.each("Boolean Number String Function Array Date RegExp Object"
				.split(" "), function(Z, Y) {
		...
		p.encodeHtml = function(Y) {
			return Y && Y.replace ? (Y.replace(/&/g, "&amp;").replace(/ /g,
		...
		p.decodeHtml = function(Y) {
			return Y && Y.replace ? (Y.replace(/&nbsp;/gi, " ").replace(
					/&lt;/gi, "<").replace(/&gt;/g, ">").replace(/&#92;/gi,
		...
		p.grep = function(Z, ae, Y) {
			var ad, aa = [], ab = 0, ac = Z.length;
			Y = !!Y;
			for (; ab < ac; ab++) {
				ad = !!ae(Z[ab], ab);
				if (Y !== ad) {
