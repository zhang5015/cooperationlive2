Site.ImageEffect = !!Site.ImageEffect ? Site.ImageEffect : {};
(function(a, c, b) {
	c.DATA = {};
...
(function(d, f, e) {
	...
	function a(v, w, l, k, n) {
		...
		case 2:
			v.hover.opacity = 1;
			v.leave.opacity = 0;
			var m = d(n).height() || l.h, t = d(n).width() || l.w;
			if (j) {
	...
	function c(j, s, A) {
		...
		var B, q, w;
		if (t == 4) {
			var h = d(A).parent(), o = -parseInt(d(A).css("marginLeft")
					.replace("px", "")) || 0;
			...
			if (t == 5) {
				...
				q = d(B).width();
				w = d(B).height();
				var m = d(B).css("margin-left"), l = parseFloat(d(A).css(
						"margin-left").replace("px", ""));
				var m = f.FUNC.TOOL.parsePixelToInt(m);
				d(j).removeClass("imageEffectsAnimate").addClass(
						"imageEffectsAnimate_Half");
				d(j).css("width", q);
				var g = d(n).height(), n = d(j).find(".props .propList"), v = d(
						j).find(".props .propBuy"), g = d(n).height(), x = parseInt(d(
						n).css("padding-top")), p = parseInt(d(n).css(
						"margin-bottom")), i = d(j).parent().find("img")
						.position().left;
				if (g > 0) {
					g += x + p
				}
...
(function(f, h, g) {
	h.FUNC.BASIC.Init = function(q) {
		...
		var i = q.moduleId, l = q.imgEffOption;
		if (!!!e(q, n)) {
			return
		}
			...
			if (j > 1 && j < 6) {
				...
				if (j == 4 || j == 5) {
					if (!Fai.isNull(n[h.DATA.KEY.OPT_CALLBACK])) {
						n[h.DATA.KEY.OPT_CALLBACK](s,
								n[h.DATA.KEY.OPT_IMG_EFF_OPTION],
								n[h.DATA.KEY.OPT_TG_OPTION],
								n[h.DATA.KEY.OPT_CALLBACK_ARGS])
					}
				}
			} else {
				if (j <= 7) {
		...
		if (j == 8) {
			...
			f(p)
					...
					.on(
							"mouseenter.imageEffectPlugs mouseleave.imageEffectPlugs",
							function(t) {
								var s = f(this).find(".imageEffects"), r = "";
								if (j < 6) {
									h.FUNC.TOOL.resetImgEff(
											n[h.DATA.KEY.OPT_TG_OPTION], this,
											s, j)
								} else {
									if (j <= 8) {
										s = f(this).find("img");
								...
								r = t.type == "mouseenter" ? "mouseenter"
										: "mouseleave";
								if (j > 1 && j <= 7) {
									h.FUNC.BUS
											.ImgEffAnimate(
													...
													n[h.DATA.KEY.OPT_IMG_EFF_OPTION],
													this,
													n[h.DATA.KEY.OPT_TG_OPTION].paramLayoutType || 0)
								}
							})
		}
		var o = f(k).find(".imageEffects");
	...
	function c(k, i) {
		var l = !Fai.isNull(f(k).find("a")) && f(k).find("a").length != 0, j;
		if (l) {
	...
	function b(i, Q) {
		...
		if (Fai.isIE6()
				&& Q[h.DATA.KEY.OPT_TG_OPTION].targetParent == "photoSmallPic_table") {
		...
		var w = f(i).find(".imageEffects"), n = (m == 2) ? "imageEffects_Border"
				: (m == 3) ? "imageEffects_Magnifier"
						: (m == 4) ? "imageEffects_FullMaks"
								: (m == 5) ? "imageEffects_HalfMaks" : "";
		var v = {
			bw : (Q[h.DATA.KEY.OPT_IMG_EFF_OPTION].borderType) ? Q[h.DATA.KEY.OPT_IMG_EFF_OPTION].borderWidth
					: 1,
			bs : (Q[h.DATA.KEY.OPT_IMG_EFF_OPTION].borderType) ? Q[h.DATA.KEY.OPT_IMG_EFF_OPTION].borderStyle
					: 1,
			bc : (Q[h.DATA.KEY.OPT_IMG_EFF_OPTION].borderType) ? Q[h.DATA.KEY.OPT_IMG_EFF_OPTION].borderColor
					: (A == 8 ? "#808080" : "#000")
		}, C = (v.bs == 1) ? "solid" : (v.bs == 2) ? "dotted"
				: (v.bs == 3) ? "dashed" : "", U = Q[h.DATA.KEY.OPT_IMG_EFF_OPTION][h.DATA.KEY.FULL_MASK_BG_STYLE], z = Q[h.DATA.KEY.OPT_IMG_EFF_OPTION][h.DATA.KEY.HALF_MASK_BG_STYLE], L = {
		...
		if (Q.moduleType == "photoCard" || Q.moduleType == "photoNewCard"
				|| Q.moduleType == "photoMoreCard") {
			r = f(i).parent();
			D = f(r).width();
			l = f(r).height();
		...
		case 5:
			...
			f(w).removeClass("imageEffectsAnimate").addClass(
					"imageEffectsAnimate_Half");
			f(w).css("bottom", "-" + l + "px");
			break;
		case 6:
	...
	function a(o, k) {
		var l = f(o).find("img"), j = k[h.DATA.KEY.OPT_IMG_EFF_OPTION], m, n;
		if (l.length == 0) {
	...
	function d(n, j) {
		var o = (j[h.DATA.KEY.OPT_IMG_EFF_OPTION].effType == 6) ? "imageEffectsAnimate_Magnify"
				: "imageEffectsAnimate_MoveLeft";
		var i = j[h.DATA.KEY.OPT_IMG_EFF_OPTION].style == 88
				|| j[h.DATA.KEY.OPT_IMG_EFF_OPTION].style == 89
				|| j[h.DATA.KEY.OPT_IMG_EFF_OPTION].style == 90;
		var m = j[h.DATA.KEY.OPT_IMG_EFF_OPTION].effType == 6
				|| j[h.DATA.KEY.OPT_IMG_EFF_OPTION].effType == 7;
		var k = j[h.DATA.KEY.OPT_TG_OPTION].picScale || 0, l = f(n).find("img");
		if (l.length == 0) {
