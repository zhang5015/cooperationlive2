...
abstract class autoCategoryActions extends sfActions
{
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    $this->cooperation_category = $this->form->getObject();
  }
  ...
  {
    $this->cooperation_category = $this->getRoute()->getObject();
    $this->form = $this->configuration->getForm($this->cooperation_category);
  }
  ...
  public function executeDelete(sfWebRequest $request)
  {
    ...
    $this->redirect('@cooperation_category');
  }
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      ...
      try {
        $cooperation_category = $form->save();
      ...
      $this->dispatcher->notify(new sfEvent($this, 'admin.save_object', array('object' => $cooperation_category)));

      if ($request->hasParameter('_save_and_add'))
      {
        $this->getUser()->setFlash('notice', $notice.' You can add another one below.');

        $this->redirect('@cooperation_category_new');
      }
      else
      {
        $this->getUser()->setFlash('notice', $notice);

        $this->redirect(array('sf_route' => 'cooperation_category_edit', 'sf_subject' => $cooperation_category));
      }
    }
    else
    {
  ...
  {
    $pager = $this->configuration->getPager('CooperationCategory');
  ...
  protected function buildQuery()
  {
    ...
    $query = Doctrine_Core::getTable('CooperationCategory')
      ->createQuery('a');

    if ($tableMethod)
    {
      $query = Doctrine_Core::getTable('CooperationCategory')->$tableMethod($query);
    }
  ...
  {
    return Doctrine_Core::getTable('CooperationCategory')->hasColumn($column);
  }
}
